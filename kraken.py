...
class Distorter:
	def __init__(self):
		...
		self.c1 = (1/512.0)
		self.c2 = (8.0 * math.pi / (1024 * 256))
		self.c3 = (math.pi / 60.0)
    ...
    #var S = Math.round(C1 * amplitude * Math.sin((C2 * frequency * y + C3 * speed * t).toFixed(6)));
		...
		s = int(round(self.c1 * amplitude * math.sin((self.c2 * frequency * y + self.c3 * speed * t))  ))
...
class BGLayer:
	...
	def gfx_draw_tile(self, pixels, stride, x,y,pal,tile,subpal,vflip,hflip):
		...
		i=0
		while i < 8:
			ttmp=self.gfx_dat['tiles'][tile][i]
